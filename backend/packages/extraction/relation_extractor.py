...
def _format_entities(entities: list[ExtractedEntity]) -> str:
    if not entities:
        return "（无）"
    return "\n".join(f"- {e.name} ({e.type_id})" for e in entities[:50])
...
async def extract_relations(
    *,
    ...
    name_to_entity: dict[str, ExtractedEntity] = {e.name: e for e in entities}
